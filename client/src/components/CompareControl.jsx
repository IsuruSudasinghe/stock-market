    ...
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
      ...
      {isCompareMode && (
        <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto">
          ...
          {mainSymbol && (
            <span className="px-2 sm:px-3 py-1 bg-primary text-white text-xs sm:text-sm font-medium rounded-full whitespace-nowrap">
              {mainSymbol}
            </span>
          )}
          ...
          {selectedStocks.map((symbol) => (
            <span
              key={symbol}
              className="px-2 sm:px-3 py-1 bg-slate-200 text-slate-700 text-xs sm:text-sm font-medium rounded-full flex items-center gap-1 whitespace-nowrap"
            >
              {symbol}
              <button
          ...
          {categoryHints.length > 0 && (
            <div className="flex items-center gap-1 px-2 py-1 bg-amber-50 border border-amber-200 rounded-full flex-wrap">
              <span className="text-xs text-amber-700 font-medium hidden sm:inline">Same category:</span>
              <span className="text-xs text-amber-700 font-medium sm:hidden">Category:</span>
          ...
          {selectedStocks.length < maxStocks - 1 && (
            <div className="relative">
              <button
                ...
                className="px-2 sm:px-3 py-1 border-2 border-dashed border-gray-300 text-gray-500 text-xs sm:text-sm font-medium rounded-full hover:border-primary hover:text-primary transition-colors flex items-center gap-1 whitespace-nowrap"
              >
              ...
              {/* Search Dropdown */}
              {showSearch && (
                <div className="absolute top-full left-0 mt-2 w-64 sm:w-72 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-[70vh] overflow-y-auto">
                    ...
                    <div className="max-h-48 overflow-y-auto">
                      {searchResults.map((company) => (
                        <button
                          key={company.symbol}
                          onClick={() => handleAddStock(company.symbol)}
                          className="w-full px-4 py-2 text-left hover:bg-slate-50 transition-colors border-b border-gray-100 last:border-0"
                        >
